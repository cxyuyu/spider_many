...
public class Content_handle {
    ...
    public static String handle(String content) {
        ...
        //时间处理
        String time = Lines.get(Lines.size() - 2);
        time = time.replace(" ", "");
        time = time.replace(" ", "");
        System.out.println("time" + time);
        if (time.contains("年") || time.contains("月") || time.contains("二"))
            save_not = 1;
        ...
        String shenpan_boss = "";
        int name_size = 0;
        if (Lines.get(Lines.size() - 3).substring(0, 3).equals("审判员")) {
            name_size = 3;
            shuji = Lines.get(Lines.size() - 1);
            shenpan = Lines.get(Lines.size() - 3);
        } else {
            name_size = 5;
            shuji = Lines.get(Lines.size() - 1);
            //书记员
            daili = Lines.get(Lines.size() - 3);
            //代理审判员
            shenpan = Lines.get(Lines.size() - 4);
            //审判员
            shenpan_boss = Lines.get(Lines.size() - 5);
            //审判长
        }
